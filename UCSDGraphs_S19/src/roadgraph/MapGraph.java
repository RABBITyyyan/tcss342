...
public class MapGraph {
	...
	{
		if (location == null || nodes_outedges.containsKey(location)) {
			return false;
		}
		
		List<MapEdge> newList = new ArrayList<MapEdge>();
		nodes_outedges.put(location, newList);
	...
	private Set<GeographicPoint> getNeighbors(GeographicPoint node) {
			...
			List<MapEdge> edges = nodes_outedges.get(node);
	...
	private class DijkstraNode implements Comparator<DijkstraNode> { 
	    public int node; 
	    public int cost; 
	...
	//		System.out.println(key + "=" + theMap.getEdges(key));
		...
		List<GeographicPoint> route = theMap1.bfs(start,end, (x) -> {});
